...
class TensorMesh(
    ...
    def edge_x_lengths(self):
        ...
        if not self.torch_active:
            ...
            if getattr(self, "_edge_x_lengths", None) is None:
                ...
                # Compute edge lengths
                if self.dim == 1:
                    edgeEx = torch.tensor(vh[0], dtype=SimpegConfig().dtype)
                elif self.dim == 2:
                    edgeEx = torch.outer(torch.tensor(vh[0], dtype=SimpegConfig().dtype), torch.ones(n[1] + 1, dtype=SimpegConfig().dtype))
                elif self.dim == 3:
                    edgeEx = np.outer(
                        vh[0], mkvc(np.outer(np.ones(n[1] + 1), np.ones(n[2] + 1)))
                    )
        ...
        if not self.torch_active:
            ...
            if getattr(self, "_edge_y_lengths", None) is None:
                ...
                if self.dim == 1:
                    ...
                    edgeEy = torch.outer(torch.ones(n[0] + 1, dtype=SimpegConfig().dtype), torch.tensor(vh[1], dtype=SimpegConfig().dtype))
                elif self.dim == 3:
                    edgeEy = np.outer(
                        np.ones(n[0] + 1), mkvc(np.outer(vh[1], np.ones(n[2] + 1)))
                    )
    ...
    def set_torch_activate_tensorM(self, framework):
        
        self.torch_active = framework
    @property
    def edge_lengths(self):  # NOQA D102
        ...
        if not self.torch_active:
            ...
            if self.dim == 1:
                return torch.tensor(self.edge_x_lengths, dtype=SimpegConfig().dtype)
            elif self.dim == 2:
                return torch.cat((torch.tensor(self.edge_x_lengths, dtype=SimpegConfig().dtype), torch.tensor(self.edge_y_lengths, dtype=SimpegConfig().dtype)))
            elif self.dim == 3:
                return np.r_[self.edge_x_lengths, self.edge_y_lengths, self.edge_z_lengths]
            return self._edge            
