...
class BaseTensorMesh(BaseRegularMesh):
    ...
    def _fastInnerProduct(
        ...
        if self._meshType == "CYL": # CYL cilindrica
        ...
        if model_size == self.nC:
            ...
            if isinstance(model, torch.Tensor):
                if not isinstance(Av, torch.Tensor):
                    ...
                    forma = coo_mat.shape
                    Av = torch.sparse_coo_tensor(indices, values, forma, dtype=SimpegConfig().dtype, device=SimpegConfig().device).coalesce()
                    

                Vprop = torch.tensor(self.cell_volumes, dtype=SimpegConfig().dtype, device=SimpegConfig().device) * mkvc(model).to(SimpegConfig().device).to(SimpegConfig().dtype)
                n_elements_tensor = torch.tensor(n_elements, dtype=SimpegConfig().dtype, device=SimpegConfig().device)
                sparse_result = torch.sparse.mm(Av.T, Vprop.unsqueeze(1)).to(SimpegConfig().dtype)
                M = n_elements_tensor * sdiag(sparse_result)
        ...
        elif model_size == self.nC * self.dim:
