...
class InnerProducts(BaseMesh):
    ...
    def _getInnerProductProjectionMatrices(self, projection_type, tensorType):
        ...
        elif SimpegConfig().torch_is_active:
            indices = torch.arange(d).repeat(2, 1)  # [0, 1, 2, 3, 4] para filas y columnas
            values = torch.ones(d)
            d_torch = torch.tensor([float(-d)], dtype=SimpegConfig().dtype)
            sparse_identity = torch.sparse_coo_tensor(indices, values, (d, d))
            V = self.sparse_kron_coo(sparse_identity, sdiag(torch.sqrt((2 ** d_torch) * torch.tensor(self.cell_volumes, dtype=SimpegConfig().dtype))))
    ...
    def _getEdgePxx(M):
        ...
        def Pxx(xEdge, yEdge):
            ...
            IND = np.r_[ind1, ind2].flatten()
            if not M.torch_is_active:
                PXX = sp.coo_matrix(
                    (np.ones(2 * M.nC), (range(2 * M.nC), IND)), shape=(2 * M.nC, M.nE)
                ).tocsr()

            else:
                row_indices = torch.arange(2 * M.nC, dtype=torch.long)
                col_indices = torch.tensor(IND, dtype=torch.long)
                # Crear tensor de índices COO (2, 2*M.nC)
                indices = torch.stack([row_indices, col_indices])
                
                # Datos con soporte para gradientes
                data = torch.ones(2 * M.nC, dtype=SimpegConfig().dtype)
                
                # Construir tensor disperso COO
                PXX = torch.sparse_coo_tensor(
                    indices, 
                    data, 
                    size=(2 * M.nC, M.nE),  # Asegurar tamaño correcto
