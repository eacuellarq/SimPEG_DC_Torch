...
class DiffOperators(BaseMesh):
    ...
    def _nodal_gradient_x_stencil(self):
        ...
        if not SimpegConfig().torch_is_active:
            if self.dim == 1:
            ...
            if self.dim == 1:
                ...
                Gx = self.sparse_kron_coo(speye(self.shape_nodes[1]), ddx(self.shape_cells[0]))
            elif self.dim == 3:
                Gx = kron3(
                    speye(self.shape_nodes[2]),
                    speye(self.shape_nodes[1]),
    ...
    def _nodal_gradient_y_stencil(self):
        ...
        if not SimpegConfig().torch_is_active:
            ...
            if self.dim == 1:
                ...
                Gy = self.sparse_kron_coo(ddx(self.shape_cells[1]), speye(self.shape_nodes[0]))
            elif self.dim == 3:
                Gy = kron3(
                    speye(self.shape_nodes[2]),
                    ddx(self.shape_cells[1]),
    ...
    def sparse_vstack(blocks):
        """
        Apila tensores dispersos de PyTorch verticalmente (por filas)

        Parámetros
        ----------
        blocks : lista de torch.Tensor
            Lista de tensores dispersos en formato COO con igual número de columnas

        Retorna
        -------
        torch.Tensor
            Tensor disperso apilado verticalmente en formato COO
        """
        # Verificar que todos los bloques tengan la misma cantidad de columnas
        n_cols = blocks[0].size(1)
        for b in blocks:
            if b.size(1) != n_cols:
                raise ValueError("Todos los bloques deben tener el mismo número de columnas")

        # Calcular offsets acumulativos de filas
        row_offsets = torch.cumsum(torch.tensor([b.size(0) for b in blocks]), dim=0)

        # Ajustar índices y concatenar
        all_indices = []
        all_values = []
        current_offset = 0
        
        for i, block in enumerate(blocks):
            # Coalescer el tensor y obtener índices/valores
            coalesced_block = block.coalesce()
            indices = coalesced_block.indices().clone()
            values = coalesced_block.values()
            
            # Ajustar índices de fila
            indices[0, :] += current_offset
            
            all_indices.append(indices)
            all_values.append(values)
            
            # Actualizar offset para el próximo bloque
            if i < len(blocks)-1:
                current_offset = row_offsets[i]

        # Crear tensor final
        final_indices = torch.cat(all_indices, dim=1)
        final_values = torch.cat(all_values)
        final_shape = (row_offsets[-1].item(), n_cols)

        return torch.sparse_coo_tensor(final_indices, final_values, final_shape).coalesce()
    ...
    def nodal_gradient(self):  # NOQA D102
        if getattr(self, "_nodal_gradient", None) is None:
            ...
            if not SimpegConfig().torch_is_active:
                ...
                rows = G.indices()[0]
                # Keep both L and rows on CPU to avoid CUDA tensor to numpy conversion error
                L_cpu = L.cpu() if isinstance(L, torch.Tensor) and L.is_cuda else L
                rows_cpu = rows.cpu() if isinstance(rows, torch.Tensor) and rows.is_cuda else rows
                scale_factors = (1 / L_cpu)[rows_cpu]
                # Ensure scale_factors is on same device as G.values()
                if isinstance(G.values(), torch.Tensor) and G.values().is_cuda:
                    if not isinstance(scale_factors, torch.Tensor):
                        scale_factors = torch.tensor(scale_factors)
                    scale_factors = scale_factors.cuda()
                scaled_values = G.values() * scale_factors
                self._nodal_gradient = torch.sparse_coo_tensor(
                    G.indices(),
                    scaled_values,
                    G.size()
                )
    ...
    def average_edge_to_cell(self):  # NOQA D102
        ...
        if SimpegConfig().torch_is_active:
            ...
            if getattr(self, "_average_edge_to_cell", None) is None:
                if self.dim == 1:
                    ...
                    self._avE2CC = 0.5 * self.torch_sparse_hstack(
                        [self.aveEx2CC, self.aveEy2CC])
                elif self.dim == 3:
        ...
        return self._avE2CC
    @staticmethod
    def torch_sparse_hstack(tensors):
        """
        Concatena horizontalmente una lista de tensores dispersos PyTorch en formato COO
        Mantiene la representación sparse sin convertir a dense
        """
        if not tensors:
            raise ValueError("Lista vacía de tensores")
        
        # Verificar que todos sean sparse y tengan misma dimensión de filas
        shapes = [t.shape for t in tensors]
        if any(len(shape) != 2 for shape in shapes):
            raise ValueError("Todos los tensores deben ser 2D")
        if any(tensors[0].shape[0] != shape[0] for shape in shapes):
            raise ValueError("Todas las matrices deben tener el mismo número de filas")

        # Acumulador de columnas para ajustar índices
        accumulated_cols = 0
        all_indices = []
        all_values = []
        
        for tensor in tensors:
            # Obtener componentes del tensor sparse
            indices = tensor.coalesce().indices()
            values = tensor.coalesce().values()
            rows, cols = indices[0], indices[1]
            
            # Ajustar índices de columnas
            adjusted_cols = cols + accumulated_cols
            adjusted_indices = torch.stack([rows, adjusted_cols])
            
            all_indices.append(adjusted_indices)
            all_values.append(values)
            
            # Actualizar acumulador de columnas
            accumulated_cols += tensor.shape[1]

        # Combinar todos los componentes
        combined_indices = torch.cat(all_indices, dim=1)
        combined_values = torch.cat(all_values)
        
        # Nueva forma: (mismas filas, suma de columnas)
        new_shape = (tensors[0].shape[0], accumulated_cols)
        ...
        if getattr(self, "_average_edge_x_to_cell", None) is None:
            ...
            if not SimpegConfig().torch_is_active: 
                ...
                if self.dim == 1:
                    ...
                    self._average_edge_x_to_cell = self.sparse_kron_coo(av(n[1]), speye(n[0]))
                elif self.dim == 3:
                    self._average_edge_x_to_cell = kron3(av(n[2]), av(n[1]), speye(n[0]))
    ...
    def average_edge_y_to_cell(self):
        ...
        if not SimpegConfig().torch_is_active:
            ...
            if getattr(self, "_average_edge_y_to_cell", None) is None:
                if self.dim == 2:
                    self._average_edge_y_to_cell = self.sparse_kron_coo(speye(n[1]), av(n[0]))
                elif self.dim == 3:
                    self._average_edge_y_to_cell = kron3(av(n[2]), speye(n[1]), av(n[0]))
        ...
        if not SimpegConfig().torch_is_active:
            ...
            if getattr(self, "_average_node_to_cell", None) is None:
                ...
                if self.dim == 1:
                    ...
                    self._average_node_to_cell = self.sparse_kron_coo(
                        av(self.shape_cells[1]), av(self.shape_cells[0])
                    )
                elif self.dim == 3:
                    self._average_node_to_cell = kron3(
