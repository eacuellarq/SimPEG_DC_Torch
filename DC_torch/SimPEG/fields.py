...
class Fields:
    ...
    def _initStore(self, name):
        ...
        if self.torch_active:
            from .config import SimpegConfig
            field = torch.zeros(self._storageShape(loc), dtype=SimpegConfig().dtype, device=SimpegConfig().device)
        else:
    ...
    def __setitem__(self, key, value):
        ...
        # print("__setitem__", self.knownFields, name, value)
        for name in newFields:
            
            if isinstance(value, torch.Tensor):
                self.torch_active=True
                field = self._initStore(name)
                # print("__setitem__a", field, newFields[name], name, ind)

                self._setField(field, newFields[name], name, ind)
            else:
                self.torch_active=False
                field = self._initStore(name)
                # print("__setitem__as", field, newFields[name], name, ind)
                self._setField(field, newFields[name], name, ind)
    ...
    def __getitem__(self, key):
        ...
        if name is None:
            ...
            return out
        # print("_getFieldaas",name, ind, src_list,)
...
class TimeFields(Fields):
    ...
    def _setField(self, field, val, name, ind):
        ...
        if self.torch_active:
            if val.numel() != np.array(shape).prod():
                raise ValueError("Incorrect size for data.")
            
            correctShape = field[:, srcInd, timeInd].shape
            
            # self.save_grad_fn_chain(val)
            field[:, srcInd, timeInd] = val.T.contiguous().reshape(correctShape[::-1]).T
    ...
    def _getField(self, name, ind, src_list):
        srcInd, timeInd = ind
        # print("_getFieldasd", srcInd, timeInd,name, ind, src_list, self._fields)
        if name in self._fields:
        ...
        if self.torch_active:
            # print("out_torch", out)
            return out.T.contiguous().reshape(shape[::-1]).T
        else:
            # print("out_numpy", out)
