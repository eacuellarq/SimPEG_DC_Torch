...
class BaseDCSimulation2D(BaseElectricalPDESimulation):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not do_trap:
            ...
            min_r = min(self.mesh.edge_lengths) if not isinstance(self.mesh.edge_lengths, torch.Tensor) else self.mesh.edge_lengths.min().item()
    ...
    def fields(self, m=None):
        ...
        # GPU Optimization 3: Memory management
        if SimpegConfig().torch_is_active and SimpegConfig().device != "cpu":
            torch.cuda.empty_cache()  # Clear GPU memory before large computation
    ...
    def dpred(self, m=None, f=None):
        ...
        if SimpegConfig().torch_is_active:
            ...
            weights = torch.tensor(weights, device=device, dtype=SimpegConfig().dtype)
            temp = torch.empty(survey.nD, device=device)
        else:
        ...
        if SimpegConfig().torch_is_active:
            ...
            # Simplified processing - each evaluation individually
            for src in survey.source_list:
                for rx in src.receiver_list:
                    eval_result = rx.eval(src, self.mesh, f)
                    # Ensure eval_result is on GPU
                    if isinstance(eval_result, torch.Tensor):
                        eval_result = eval_result.to(device)
                    else:
                        eval_result = torch.tensor(eval_result, device=device, dtype=SimpegConfig().dtype)
                    
                    # Use appropriate multiplication based on dimensions
                    if eval_result.dim() == 1:
                        # Vector dot product
                        d = torch.dot(eval_result, weights)
                    elif eval_result.dim() == 2:
                        # Matrix-vector multiplication
                        d = torch.matmul(eval_result, weights)
                    else:
                        # Fallback to matmul
                        d = torch.matmul(eval_result, weights)
                    
                    # Ensure d is at least 1D for concatenation and has correct dtype
                    if d.dim() == 0:
                        d = d.unsqueeze(0)
                    elif d.dim() > 1:
                        d = d.flatten()
                    # Ensure correct dtype
                    d = d.to(SimpegConfig().dtype)
                    d_list.append(d)
            
            temp = torch.cat(d_list) if d_list else torch.empty(0, device=device, dtype=SimpegConfig().dtype)
        else:
            for src in survey.source_list:
                for rx in src.receiver_list:
...
class Simulation2DNodal(BaseDCSimulation2D):
    ...
    def __init__(self, mesh, survey=None, bc_type="Robin",  torch_is_active=False, **kwargs):
        super().__init__(mesh=mesh, survey=survey, **kwargs)
        self.solver_opts["is_symmetric"] = True
        self.solver_opts["is_positive_definite"] = True
        self.bc_type = bc_type
        self.torch_is_active = torch_is_active
    ...
    def getA(self, ky):
        ...
        A = D MfRhoI G
        """
        # CUDA Memory & Timing - getA Start
        import time
        time_getA_start = time.time()
        # To handle Mixed boundary condition
        if not self.torch_is_active:
            self.setBC(ky=ky)

        else:
            self.setBC(ky=ky)

        MeSigma = self.MeSigma # Los gradientes van bien
        ...
        if isinstance(Grad, torch.Tensor):
            GradT = GradT.to(SimpegConfig().dtype)
            MeSigma = MeSigma.to(SimpegConfig().dtype)
            Grad = Grad.to(SimpegConfig().dtype)
            MnSigma = MnSigma.to(SimpegConfig().dtype)
            ky_tensor = torch.tensor(ky, dtype=SimpegConfig().dtype, device=SimpegConfig().device)
