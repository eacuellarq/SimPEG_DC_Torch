...
class BaseRx(BaseSimPEGRx):
    ...
    def eval(self, src, mesh, f):
        ...
        if f.torch_active:
            # Ensure both P and f have consistent dtypes
            f_field = f[src, proj_f].to(SimpegConfig().dtype)

            # Ensure proper dimensions for sparse matrix multiplication
            if f_field.dim() == 1:
                f_field = f_field.unsqueeze(1)

            # v= torch_sparse.spmm(index=indices, value=values, m=P.shape[0], n=P.shape[1], matrix= f_field).squeeze()
            v = torch.sparse.mm(P, f_field).squeeze()
