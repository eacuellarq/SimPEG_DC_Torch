...
class Simulation3DNodal(BaseDCSimulation):
    ...
    def scipy_sparse_to_torch(self, sparse_matrix, dtype=SimpegConfig().dtype):
        """
        Convert scipy sparse matrix (CSR, CSC, or COO) to a coalesced PyTorch sparse tensor.

        Parameters:
        sparse_matrix : scipy.sparse.csr_matrix, csc_matrix, or coo_matrix
            Input sparse matrix from scipy.
        dtype : torch.dtype, optional
            Desired data type of the output tensor (default=torch.float32).

        Returns:
        torch.sparse.Tensor
            Coalesced sparse tensor in COO format.

        Raises:
        TypeError: If input is not a scipy CSR, CSC, or COO sparse matrix.
        """

        if not isinstance(sparse_matrix, (sp.csr_matrix, sp.csc_matrix, sp.coo_matrix)):
            raise TypeError(
                f"Expected scipy.sparse.csr_matrix, csc_matrix, or coo_matrix. "
                f"Got {type(sparse_matrix)}"
            )

        # Convert all formats to COO for consistent handling
        coo_matrix = sparse_matrix.tocoo()

        # Extract indices and values
        row = torch.from_numpy(coo_matrix.row.astype(np.int64))
        col = torch.from_numpy(coo_matrix.col.astype(np.int64))
        indices = torch.stack([row, col], dim=0)
        values = torch.tensor(coo_matrix.data, dtype=dtype)
    ...
    def torch_diags(self, diagonals, offsets=0, shape=None, dtype=None, device=None):
        """
        Constructs a sparse 2D tensor (matrix) from specified diagonals.

        Parameters:
        diagonals : list of torch.Tensor
            Sequence of 1D tensors containing diagonal values.
        offsets : list of int or int
            Diagonal offsets. Positive values indicate upper diagonals,
            negative values indicate lower diagonals, and 0 is the main diagonal.
        shape : tuple of int, optional
            Shape of the output tensor (rows, columns). If None, the minimal
            required shape is calculated automatically.
        dtype : torch.dtype, optional
            Data type of the output tensor. Uses the first diagonal's dtype if None.
        device : torch.device, optional
            Device of the output tensor (e.g., 'cpu', 'cuda'). Uses the first
            diagonal's device if None.

        Returns:
        torch.sparse.Tensor
            Sparse tensor in COO format containing the specified diagonals.

        Notes:
        - Repeated offsets are not supported (as in SciPy).
        - The output is always a sparse tensor in COO format.
        """
        # Validate inputs
        if not isinstance(offsets, list):
            offsets = [offsets]

        if isinstance(diagonals, torch.Tensor):
            diagonals = [diagonals]
        if len(diagonals) != len(offsets):
            raise ValueError("Number of diagonals must match number of offsets")
        
        # Determine dtype and device from diagonals if not provided
        dtype = diagonals[0].dtype if dtype is None else dtype
        device = diagonals[0].device if device is None else device
        
        # Convert diagonals to specified dtype/device
        diagonals = [d.to(device).to(dtype) for d in diagonals]
        
        # Calculate minimal shape if not provided
        if shape is None:
            min_row, min_col = 0, 0
            max_row, max_col = 0, 0
        ...
        for diag, k in zip(diagonals, offsets):
            n = len(diag)
            if k <= 0:
                # Lower diagonal: row = column - k (shifted down by |k|)
                r = torch.arange(-k, -k + n, device=device)
                c = torch.arange(0, n, device=device)
            else:
                # Upper diagonal: column = row + k (shifted right by k)
                r = torch.arange(0, n, device=device)
                c = torch.arange(k, k + n, device=device)
            
            # Filter indices within matrix bounds
            valid = (r < shape[0]) & (c < shape[1])
            r, c = r[valid], c[valid]
            v = diag[valid]
        ...
        # Concatenate indices/values
        if not values:
            # Handle empty diagonals (create zero sparse tensor)
    ...
    def getA(self, resistivity=None):
        ...
        if SimpegConfig().torch_is_active:
            if resistivity is None:
                MeSigma = self.MeSigma.to(SimpegConfig().dtype)
            else:
                
                MeSigma = self.mesh.get_edge_inner_product(1.0 / resistivity).to(SimpegConfig().dtype)
        else:
            if resistivity is None:
        ...
        if SimpegConfig().torch_is_active:
            Grad = self.scipy_sparse_to_torch(Grad).to(SimpegConfig().dtype)
        ...
        if self.bc_type == "Neumann":
            ...
            try:
                if SimpegConfig().torch_is_active:
                    ...
                    avgbc = self.scipy_sparse_to_torch(self._AvgBC).to(SimpegConfig().dtype).coalesce()
                    
                    # avgbc_index = avgbc.indices()
                    # avgbc_values = avgbc.values()

                    # diags = torch_sparse.spmm(index=avgbc_index, value=avgbc_values, m=avgbc.shape[0],
                    #                           n = avgbc.shape[1], matrix = self.sigma.unsqueeze(1)).flatten()

                    A = A + self.torch_diags(avgbc @ self.sigma)
