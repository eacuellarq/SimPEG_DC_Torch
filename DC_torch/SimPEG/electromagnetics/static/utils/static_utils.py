...
def apparent_resistivity_from_voltage(
    ...
    if isinstance(volts, torch.Tensor):
        from SimPEG.config import SimpegConfig
        G = torch.tensor(G, dtype=SimpegConfig().dtype, device=volts.device)
        eps_tensor = torch.tensor(eps, dtype=SimpegConfig().dtype, device=volts.device)
        rhoApp = torch.abs(volts * (1.0 / (G + eps_tensor)))
    else:
