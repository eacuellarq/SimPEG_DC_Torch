...
class BaseSimulation(props.HasModel):
    ...
    def make_synthetic_data(
        ...
        if add_noise is True:
            if isinstance(dclean, torch.Tensor):

                
                eps = 1e-8
                std = torch.sqrt((relative_error * torch.abs(dclean))**2 + noise_floor**2 + eps)
                noise = std * torch.randn(
                    dclean.shape,
                    dtype=dclean.dtype,     # Mantener float32/float64
                    device=dclean.device    # Asegurar mismo dispositivo que dclean
                )
                dobs = dclean + noise
