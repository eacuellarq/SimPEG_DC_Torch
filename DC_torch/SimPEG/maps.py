...
class IdentityMap:
    ...
    def __mul__(self, val):
        """
        Modified SimPEG Maps for PyTorch Integration

        Changelog:
        - [2024-03-15] (v2.1) - Added Torch support for IdentityMap

        Author: C55
        Compatibility: SimPEG 0.18.1, PyTorch 2.0+
        """
        ...
        elif isinstance(val, torch.Tensor):  # <--- Manejar torch.Tensor
            
            if self.shape[1] != "*" and self.shape[1] != val.shape[0]:
                raise ValueError(f"Dimension mismatch between map {self.shape} and tensor {val.shape}")
...
class InjectActiveCells(IdentityMap):
    ...
    def _transform(self, m):
        if isinstance(self.P, csr):
            self.P_numpy = self.P
     
        if SimpegConfig().torch_is_active:
            if not isinstance(self.P, torch.Tensor):
                coo_mat = self.P.tocoo()
            ...
            return torch.sparse.mm(self.P, m.unsqueeze(1)).to(dtype=SimpegConfig().dtype) + torch.tensor(self.valInactive, dtype=SimpegConfig().dtype).unsqueeze(1)
        self.P = self.P_numpy
